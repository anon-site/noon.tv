...
// Auto-save to channels.json file functions
function saveToChannelsJson() {
    try {
        const data = {
        ...
        console.log('✅ تم حفظ البيانات في ملف channels.json');
        app.notifySuccess('تم حفظ البيانات في ملف channels.json');
        return true;
        
    } catch (error) {
        console.error('❌ خطأ في حفظ ملف channels.json:', error);
        app.notifyError('خطأ في حفظ ملف channels.json: ' + error.message);
        return false;
    }
}

// Load from channels.json file
async function loadFromChannelsJson(file) {
    try {
        ...
        if (!data.channels || !Array.isArray(data.channels)) {
            throw new Error('ملف channels.json غير صالح - لا يحتوي على قنوات');
        }
        ...
        console.log('✅ تم تحميل البيانات من ملف channels.json بنجاح');
        app.notifySuccess(`تم تحميل ${app.channels.length} قناة من ملف channels.json`);
        return true;
        
    } catch (error) {
        console.error('❌ خطأ في تحميل ملف channels.json:', error);
        app.notifyError('خطأ في تحميل ملف channels.json: ' + error.message);
        return false;
    }
}

// Auto-save function that triggers on channel changes
function autoSaveToJson() {
    // Only save if there are channels
...
function toggleAutoSave() {
    ...
    const button = document.querySelector('.auto-save-btn');
    const text = document.getElementById('autoSaveText');
    
    if (autoSaveEnabled) {
        button.classList.add('active');
        text.textContent = 'إيقاف الحفظ التلقائي';
        app.notifySuccess('تم تفعيل الحفظ التلقائي في ملف channels.json');
    } else {
        button.classList.remove('active');
        text.textContent = 'تفعيل الحفظ التلقائي';
        app.notifyInfo('تم إيقاف الحفظ التلقائي في ملف channels.json');
    }
}

// Update last modification time
function updateLastModificationTime() {
    const lastModElement = document.getElementById('lastJsonModification');
    if (lastModElement) {
        const now = new Date();
        lastModElement.textContent = now.toLocaleString('ar-SA');
    }
}

// Hook into channel modification functions to trigger auto-save
function hookIntoChannelModifications() {
    // Override addChannel function
    const originalAddChannel = app.addChannel;
    app.addChannel = function(channelData) {
        const result = originalAddChannel.call(this, channelData);
    ...
    // Override editChannel function
    const originalEditChannel = app.editChannel;
    app.editChannel = function(channelId, event) {
        const result = originalEditChannel.call(this, channelId, event);
        if (result && autoSaveEnabled) {
            autoSaveToJson();
            updateLastModificationTime();
        }
    ...
    // Override deleteChannel function
    const originalDeleteChannel = app.deleteChannel;
    app.deleteChannel = function(channelId) {
        const result = originalDeleteChannel.call(this, channelId);
        if (result && autoSaveEnabled) {
            autoSaveToJson();
            updateLastModificationTime();
        }
    ...
    // Override toggleFavorite function
    const originalToggleFavorite = app.toggleFavorite;
    app.toggleFavorite = function(channelId) {
        const result = originalToggleFavorite.call(this, channelId);
        if (autoSaveEnabled) {
            autoSaveToJson();
            updateLastModificationTime();
        }
...
// Initialize JSON management when app is ready
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        if (window.app) {
            hookIntoChannelModifications();
            updateLastModificationTime();
        }
    }, 1000);
});
