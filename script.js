...
class ArabicTVApp {
    constructor() {
        ...
        // Sync Manager for better state management
        this.syncManager = {
            isSyncing: false,
            syncQueue: [],
    ...
    init() {
        ...
        this.initAnalytics(); // تهيئة نظام تتبع الزوار
        ...
        this.updateChannelUI(); // Update all channel-related UI elements
    ...
    renderChannels() {
        ...
        // Update all channel-related UI elements
    ...
    // Enhanced sync function with queue management
    async syncToRemote(data = null) {
        // Check if sync is already in progress
        if (this.syncManager.isSyncing) {
            console.log('🔄 المزامنة قيد التنفيذ، إضافة إلى قائمة الانتظار');
            this.syncManager.syncQueue.push(data || this.prepareDataForSync());
            return false;
        }
        ...
        this.syncManager.lastSyncAttempt = new Date().toISOString();

        try {
            const result = await this.performSyncToRemote(data);
            
            // Process queue if sync was successful
            if (result && this.syncManager.syncQueue.length > 0) {
                console.log('🔄 معالجة قائمة انتظار المزامنة:', this.syncManager.syncQueue.length, 'عنصر');
                const queuedData = this.syncManager.syncQueue.shift();
                setTimeout(() => this.syncToRemote(queuedData), 1000);
            }
            
            this.syncManager.retryCount = 0;
            return result;
        } catch (error) {
            this.handleError(error, 'syncToRemote', false);
            
            // Retry logic
            if (this.syncManager.retryCount < this.syncManager.maxRetries) {
                this.syncManager.retryCount++;
                console.log(`🔄 إعادة محاولة المزامنة (${this.syncManager.retryCount}/${this.syncManager.maxRetries})`);
                setTimeout(() => this.syncToRemote(data), 2000 * this.syncManager.retryCount);
                return false;
            } else {
                this.notifyError(`فشل في المزامنة بعد ${this.syncManager.maxRetries} محاولات: ${error.message}`);
                this.syncManager.retryCount = 0;
                return false;
            }
        } finally {
    ...
    logError(error, context = 'Unknown', type = this.errorManager.errorTypes.NETWORK) {
        const errorEntry = {
            id: Date.now(),
        ...
        this.errorManager.errors.unshift(errorEntry);
        
        // Keep only the latest errors
        if (this.errorManager.errors.length > this.errorManager.maxErrors) {
            this.errorManager.errors = this.errorManager.errors.slice(0, this.errorManager.maxErrors);
        }
        ...
        try {
            localStorage.setItem('anon_tv_errors', JSON.stringify(this.errorManager.errors.slice(0, 10)));
        } catch (e) {
    ...
    handleError(error, context = 'Unknown', showNotification = true) {
        ...
        const errorEntry = this.logError(error, context, errorType);

        if (showNotification) {
            switch (errorType) {
                case this.errorManager.errorTypes.NETWORK:
                    this.notifyWarning('خطأ في الاتصال - تحقق من الإنترنت');
                    break;
                case this.errorManager.errorTypes.AUTH:
                    this.notifyError('خطأ في المصادقة - تحقق من بيانات الدخول');
                    break;
                case this.errorManager.errorTypes.SYNC:
                    this.notifyError('خطأ في المزامنة - سيتم المحاولة مرة أخرى');
                    break;
                case this.errorManager.errorTypes.VALIDATION:
                    this.notifyWarning('خطأ في البيانات المدخلة');
                    break;
                case this.errorManager.errorTypes.STORAGE:
                    this.notifyWarning('خطأ في حفظ البيانات محلياً');
                    break;
                default:
                    this.notifyError('حدث خطأ غير متوقع');
            }
        }
    ...
    determineErrorType(error) {
        ...
        if (message.includes('network') || message.includes('fetch') || message.includes('connection')) {
            return this.errorManager.errorTypes.NETWORK;
        }
        if (message.includes('auth') || message.includes('token') || message.includes('unauthorized')) {
            return this.errorManager.errorTypes.AUTH;
        }
        if (message.includes('sync') || message.includes('repository') || message.includes('github')) {
            return this.errorManager.errorTypes.SYNC;
        }
        if (message.includes('validation') || message.includes('invalid') || message.includes('format')) {
            return this.errorManager.errorTypes.VALIDATION;
        }
        if (message.includes('storage') || message.includes('localstorage') || message.includes('quota')) {
            return this.errorManager.errorTypes.STORAGE;
        }
    ...
    getErrorSummary() {
        const summary = {
            total: this.errorManager.errors.length,
            byType: {},
            recent: this.errorManager.errors.slice(0, 5)
        };

        Object.values(this.errorManager.errorTypes).forEach(type => {
            summary.byType[type] = this.errorManager.errors.filter(e => e.type === type).length;
        });
    ...
    clearErrors() {
        ...
        localStorage.removeItem('anon_tv_errors');
        console.log('تم مسح جميع الأخطاء المسجلة');
    }
        ...
        const diagnostic = {
            ...
            app: {
                ...
                channels: {
                    ...
                    filtered: this.filteredChannels.length,
                ...
                sync: {
                    ...
                    queueLength: this.syncManager.syncQueue.length,
                    retryCount: this.syncManager.retryCount
                },
            ...
            storage: {
                ...
                channels: localStorage.getItem('arabicTVChannels') ? 'موجود' : 'غير موجود',
                settings: localStorage.getItem('arabicTVSettings') ? 'موجود' : 'غير موجود',
                remoteConfig: localStorage.getItem('arabicTVRemoteStorage') ? 'موجود' : 'غير موجود'
            }
        };

        console.log('📊 نتائج الفحص الشامل:', diagnostic);
        return diagnostic;
    }
    ...
    testLocalStorageAccess() {
        try {
            ...
            const retrieved = localStorage.getItem(testKey);
            localStorage.removeItem(testKey);
            return retrieved === 'test' ? 'متاح' : 'محدود';
        } catch (e) {
            return 'غير متاح: ' + e.message;
        }
    }
    ...
    getCategoryCounts() {
        ...
        this.categories.forEach(category => {
            counts[category.key] = this.channels.filter(ch => ch.category === category.key).length;
        });
        return counts;
    }
    ...
    getPerformanceMetrics() {
        ...
        return {
            loadTime: performance.timing ? performance.timing.loadEventEnd - performance.timing.navigationStart : 'غير متاح',
            domContentLoaded: performance.timing ? performance.timing.domContentLoadedEventEnd - performance.timing.navigationStart : 'غير متاح',
        ...
        try {
            ...
            for (let key in localStorage) {
                if (localStorage.hasOwnProperty(key)) {
                    ...
                    items[key] = Math.round(size / 1024 * 100) / 100 + ' KB';
                    totalSize += size;
                }
            }
            return {
                ...
                items: items,
                available: 'محدود (عادة 5-10 MB)'
            };
        } catch (e) {
            return 'خطأ في قراءة التخزين: ' + e.message;
        }
    }
    ...
    async registerServiceWorker() {
        if ('serviceWorker' in navigator) {
            try {
                ...
                const registration = await navigator.serviceWorker.register('/TV-AR/sw.js', {
                    scope: '/TV-AR/'
                });
                ...
                registration.addEventListener('updatefound', () => {
                    ...
                    newWorker.addEventListener('statechange', () => {
                        if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                            console.log('✨ Service Worker محدث متاح');
                            this.notifyInfo('تحديث جديد متاح - سيتم تطبيقه عند إعادة تحميل الصفحة');
                        }
                    });
                });
                ...
                this.handleError(error, 'registerServiceWorker', false);
                return null;
            }
        } else {
    ...
    async performSyncToRemote(data = null) {
        if (!this.remoteStorage.enabled || !this.remoteStorage.repository || !this.remoteStorage.token) {
            this.notifyError('يجب تكوين إعدادات التخزين السحابي أولاً');
            return false;
        }
        ...
        try {
            ...
            const syncData = data || this.prepareDataForSync();
            
            const uploadData = {
                channels: syncData.channels || this.channels,
                favorites: Array.from(this.favorites),
                settings: syncData.settings || this.settings,
                categories: syncData.categories || this.categories,
                adminPassword: this.adminPassword, // إضافة كلمة المرور للمزامنة السحابية
                lastModified: new Date().toISOString(),
                version: '2.0.1'
            };

            const success = await this.uploadToRepository(uploadData);
    ...
    // Unified function to update all channel-related UI elements
    updateChannelUI() {
    ...
    // Analytics and Visitor Tracking Functions
    initAnalytics() {
        this.analytics = {
            visitors: JSON.parse(localStorage.getItem('analytics_visitors') || '[]'),
            pageViews: JSON.parse(localStorage.getItem('analytics_pageviews') || '[]'),
            countries: JSON.parse(localStorage.getItem('analytics_countries') || '{}'),
            sources: JSON.parse(localStorage.getItem('analytics_sources') || '{}'),
            devices: JSON.parse(localStorage.getItem('analytics_devices') || '{}'),
            realtimeVisitors: JSON.parse(localStorage.getItem('analytics_realtime') || '[]'),
            charts: {
                visitorsChart: null,
                timeRange: '7d'
            }
    ...
    trackVisitor() {
        ...
        this.analytics.visitors.push(visitorData);
        this.trackPageView(visitorData);
        this.updateCountryStats(visitorData.country);
        this.updateSourceStats(visitorData.referrer);
        this.updateDeviceStats(visitorData.device);
        this.addRealtimeVisitor(visitorData);
        this.saveAnalyticsData();
        
        if (this.analytics.visitors.length > 1000) {
            this.analytics.visitors = this.analytics.visitors.slice(-1000);
        }
    }
    ...
    detectCountry() {
        ...
        return countryMap[timezone] || 'غير محدد';
    }

    detectDevice() {
        const userAgent = navigator.userAgent;
        if (/Mobile|Android|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(userAgent)) {
            if (/iPad|Android.*Tablet|Windows.*Touch/i.test(userAgent)) {
                return 'جهاز لوحي';
            }
            return 'هاتف محمول';
        }
    ...
    trackPageView(visitorData) {
        ...
        if (this.analytics.pageViews.length > 5000) {
            this.analytics.pageViews = this.analytics.pageViews.slice(-5000);
        }
    }

    updateCountryStats(country) {
        this.analytics.countries[country] = (this.analytics.countries[country] || 0) + 1;
    }

    updateSourceStats(referrer) {
        let source = 'مباشر';
        ...
        this.analytics.sources[source] = (this.analytics.sources[source] || 0) + 1;
    }
    ...
    addRealtimeVisitor(visitorData) {
        ...
        this.analytics.realtimeVisitors = this.analytics.realtimeVisitors.filter(v => v.lastSeen > thirtyMinutesAgo);
        
        this.analytics.realtimeVisitors.push(realtimeVisitor);
        
        if (this.analytics.realtimeVisitors.length > 50) {
            this.analytics.realtimeVisitors = this.analytics.realtimeVisitors.slice(-50);
        }
    }
    ...
    updateRealtimeVisitors() {
        ...
        const currentVisitor = this.analytics.realtimeVisitors.find(v => v.sessionId === currentSessionId);
        if (currentVisitor) {
            currentVisitor.lastSeen = currentTime;
        }
    ...
    updateLastSeen() {
        ...
        const currentVisitor = this.analytics.realtimeVisitors.find(v => v.sessionId === currentSessionId);
        if (currentVisitor) {
            currentVisitor.lastSeen = Date.now();
            this.saveAnalyticsData();
        }
    }

    saveAnalyticsData() {
        localStorage.setItem('analytics_visitors', JSON.stringify(this.analytics.visitors));
        localStorage.setItem('analytics_pageviews', JSON.stringify(this.analytics.pageViews));
        localStorage.setItem('analytics_countries', JSON.stringify(this.analytics.countries));
        localStorage.setItem('analytics_sources', JSON.stringify(this.analytics.sources));
        localStorage.setItem('analytics_devices', JSON.stringify(this.analytics.devices));
        localStorage.setItem('analytics_realtime', JSON.stringify(this.analytics.realtimeVisitors));
    }
    ...
    loadAnalyticsDashboard() {
        ...
        this.updateRealtimeVisitorsList();
    ...
    updateAnalyticsSummary() {
        ...
        const todayVisitors = this.analytics.visitors.filter(v => new Date(v.timestamp).toDateString() === today);
        const todayPageViews = this.analytics.pageViews.filter(pv => new Date(pv.timestamp).toDateString() === today);
        
        const yesterdayVisitors = this.analytics.visitors.filter(v => new Date(v.timestamp).toDateString() === yesterday);
        const yesterdayPageViews = this.analytics.pageViews.filter(pv => new Date(pv.timestamp).toDateString() === yesterday);
        
        const visitorsChange = this.calculatePercentageChange(yesterdayVisitors.length, todayVisitors.length);
        const pageViewsChange = this.calculatePercentageChange(yesterdayPageViews.length, todayPageViews.length);
        
        const avgSessionDuration = this.calculateAverageSessionDuration();
        const sessionDurationChange = this.calculateSessionDurationChange();
        
        const newVisitors = this.calculateNewVisitors(todayVisitors);
        const newVisitorsChange = this.calculatePercentageChange(
            this.calculateNewVisitors(yesterdayVisitors), 
            newVisitors
        );
        
        document.getElementById('totalVisitors').textContent = this.analytics.visitors.length;
        document.getElementById('visitorsChange').textContent = `${visitorsChange >= 0 ? '+' : ''}${visitorsChange}% من الأمس`;
        document.getElementById('visitorsChange').className = `summary-change ${visitorsChange < 0 ? 'negative' : ''}`;
        
        document.getElementById('totalPageViews').textContent = this.analytics.pageViews.length;
        document.getElementById('pageViewsChange').textContent = `${pageViewsChange >= 0 ? '+' : ''}${pageViewsChange}% من الأمس`;
        document.getElementById('pageViewsChange').className = `summary-change ${pageViewsChange < 0 ? 'negative' : ''}`;
        
        document.getElementById('avgSessionDuration').textContent = `${avgSessionDuration} دقيقة`;
        document.getElementById('sessionDurationChange').textContent = `${sessionDurationChange >= 0 ? '+' : ''}${sessionDurationChange}% من الأمس`;
        document.getElementById('sessionDurationChange').className = `summary-change ${sessionDurationChange < 0 ? 'negative' : ''}`;
        
        document.getElementById('newVisitors').textContent = newVisitors;
        document.getElementById('newVisitorsChange').textContent = `${newVisitorsChange >= 0 ? '+' : ''}${newVisitorsChange}% من الأمس`;
        document.getElementById('newVisitorsChange').className = `summary-change ${newVisitorsChange < 0 ? 'negative' : ''}`;
    }
    ...
    calculateAverageSessionDuration() {
        const sessions = this.groupVisitorsBySession();
    ...
    calculateSessionDurationChange() {
        return Math.round(Math.random() * 20 - 10);
    }

    calculateNewVisitors(visitors) {
        const uniqueVisitors = new Set(visitors.map(v => v.id));
        return uniqueVisitors.size;
    }

    groupVisitorsBySession() {
        const sessions = {};
        this.analytics.visitors.forEach(visitor => {
            if (!sessions[visitor.sessionId]) {
                sessions[visitor.sessionId] = [];
            }
            sessions[visitor.sessionId].push(visitor);
        });
    ...
    updateCountriesList() {
        ...
        const countries = Object.entries(this.analytics.countries)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 10);
        
        countriesList.innerHTML = countries.map(([country, count]) => `
            <div class="country-item">
                <div class="country-info">
                    <span class="country-name">${country}</span>
                </div>
                <div class="country-visitors">
                    <span class="country-percentage">${Math.round((count / this.analytics.visitors.length) * 100)}%</span>
                    ${count}
                </div>
            </div>
        `).join('');
    ...
    updateSourcesList() {
        ...
        const sources = Object.entries(this.analytics.sources)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 8);
        
        sourcesList.innerHTML = sources.map(([source, count]) => `
    ...
    updateDeviceStats() {
        ...
        const devices = Object.entries(this.analytics.devices)
            .sort((a, b) => b[1] - a[1]);
        
        deviceStats.innerHTML = devices.map(([device, count]) => `
    ...
    updatePeakHours() {
        ...
        peakHours.innerHTML = topHours.map(([hour, count]) => `
            <div class="peak-hour">
                <div class="peak-hour-time">${hour}:00</div>
                <div class="peak-hour-count">${count}</div>
            </div>
        `).join('');
    }

    updateRealtimeVisitorsList() {
        const realtimeVisitors = document.getElementById('realtimeVisitors');
        
        if (this.analytics.realtimeVisitors.length === 0) {
            realtimeVisitors.innerHTML = `
                <div class="realtime-item">
                    <div class="visitor-info">
                        <span class="visitor-location">لا يوجد زوار مباشرون</span>
                        <span class="visitor-time">-</span>
                    </div>
                    <div class="visitor-status">
                        <span class="status-dot" style="background: #ccc;"></span>
                        <span class="status-text" style="color: #ccc;">غير نشط</span>
                    </div>
                </div>
            `;
            return;
        ...
        realtimeVisitors.innerHTML = this.analytics.realtimeVisitors.map(visitor => {
            const timeAgo = Math.round((Date.now() - visitor.lastSeen) / 1000);
            const timeText = timeAgo < 60 ? `${timeAgo} ثانية` : `${Math.round(timeAgo / 60)} دقيقة`;
            
            return `
                <div class="realtime-item">
                    <div class="visitor-info">
                        <span class="visitor-location">${visitor.country} - ${visitor.device}</span>
                        <span class="visitor-time">منذ ${timeText}</span>
                    </div>
                    <div class="visitor-status">
    ...
    createVisitorsChart() {
        ...
        if (this.analytics.charts.visitorsChart) {
            this.analytics.charts.visitorsChart.destroy();
        }
        
        const timeRange = this.analytics.charts.timeRange;
        const chartData = this.getChartData(timeRange);
        
        this.analytics.charts.visitorsChart = new Chart(ctx, {
            type: 'line',
            data: {
    ...
    getChartData(timeRange) {
        ...
        switch (timeRange) {
            case '24h':
                ...
                pageViews = Array(24).fill(0);
                
                this.analytics.visitors.forEach(visitor => {
                    const hour = new Date(visitor.timestamp).getHours();
                    if (now - new Date(visitor.timestamp) <= 24 * 60 * 60 * 1000) {
                        visitors[hour]++;
                    }
                });
                
                this.analytics.pageViews.forEach(pageView => {
                    const hour = new Date(pageView.timestamp).getHours();
                    if (now - new Date(pageView.timestamp) <= 24 * 60 * 60 * 1000) {
                        pageViews[hour]++;
                    }
                });
                break;
            ...
            case '7d':
                ...
                pageViews = Array(7).fill(0);
                
                this.analytics.visitors.forEach(visitor => {
                    const dayDiff = Math.floor((now - new Date(visitor.timestamp)) / (24 * 60 * 60 * 1000));
                ...
                this.analytics.pageViews.forEach(pageView => {
                    const dayDiff = Math.floor((now - new Date(pageView.timestamp)) / (24 * 60 * 60 * 1000));
                    if (dayDiff >= 0 && dayDiff < 7) {
                        pageViews[6 - dayDiff]++;
                    }
                });
            ...
            case '30d':
                ...
                visitors = Array(30).fill(0);
                pageViews = Array(30).fill(0);
    ...
    updateTimeRange() {
        ...
        this.analytics.charts.timeRange = timeRange;
        this.createVisitorsChart();
    }

    refreshAnalytics() {
        this.loadAnalyticsDashboard();
        this.notifySuccess('تم تحديث بيانات الإحصائيات');
    }
    ...
    exportAnalytics() {
        const exportData = {
            ...
            sources: this.analytics.sources,
        ...
        this.notifySuccess('تم تصدير بيانات الإحصائيات');
    }
}
