...
class ArabicTVApp {
    ...
    async playChannel(channel) {
        ...
        const type = channel.type || (this.isYouTubeUrl(channel.url) ? 'youtube' : 
            (channel.url.includes('/play/') || channel.url.includes(':8000/') || channel.url.includes(':8080/') || channel.url.includes(':1935/') || channel.url.includes(':8081/') || channel.url.includes(':8082/')) ? 'iptv' : 'hls');
    ...
    detectUrlType() {
        ...
        } else if (url.includes('/play/') || url.includes(':8000/') || url.includes(':8080/') || url.includes(':1935/') || url.includes(':8081/') || url.includes(':8082/')) {
            urlType = 'iptv';
            iconClass = 'fas fa-tv';
            typeText = 'IPTV مباشر';
    ...
    async loadIPTVStream(url) {
        ...
        try {
            ...
            loading.innerHTML = `
                <div class="spinner"></div>
                <p>جارٍ تحميل البث...</p>
                <small style="margin-top: 10px; color: #666;">إذا لم يعمل البث، سيتم تجربة طرق بديلة تلقائياً</small>
            `;
            ...
            try {
                ...
                console.log('الرابط متاح:', url);
            } catch (fetchError) {
                console.warn('لا يمكن الوصول للرابط مباشرة:', fetchError);
            }
            
            // Try to detect the stream type
            ...
            console.log('نوع البث المكتشف:', streamType);
            
            // First, try direct video source
            try {
                source.src = url;
                
                // Try to detect the best MIME type for the stream
                if (url.includes('.ts') || url.includes('ts/')) {
                    source.type = 'video/mp2t';
                } else if (url.includes('.mp4') || url.includes('mp4/')) {
                    source.type = 'video/mp4';
                } else if (url.includes('.webm') || url.includes('webm/')) {
                    source.type = 'video/webm';
                } else if (url.includes('.avi') || url.includes('avi/')) {
                    source.type = 'video/x-msvideo';
                } else if (url.includes('.mov') || url.includes('mov/')) {
                    source.type = 'video/quicktime';
                } else {
                    // Default to mp4 for IPTV streams
                    source.type = 'video/mp4';
                }
                
                video.load();
                streamLoaded = true;
                console.log('تم تحميل البث مباشرة');
                
            } catch (directError) {
                console.warn('فشل التحميل المباشر، جاري المحاولة بـ HLS:', directError);
                ...
                if (typeof Hls !== 'undefined' && Hls.isSupported()) {
                    ...
                    this.hls.on(Hls.Events.MANIFEST_PARSED, () => {
                        console.log('تم تحميل البث بـ HLS');
                        streamLoaded = true;
                        loading.style.display = 'none';
                    ...
                    this.hls.on(Hls.Events.ERROR, (event, data) => {
                        ...
                        this.showVideoError('خطأ في تشغيل البث - تحقق من الرابط');
                    });
                }
            }
            ...
            if (!streamLoaded) {
                ...
                console.log('جاري المحاولة بطرق بديلة...');
                
                try {
                    // Try iframe first
                    this.tryIframeFallback(url);
                    streamLoaded = true;
                } catch (iframeError) {
                    console.warn('فشل iframe:', iframeError);
                    
                    try {
                        // Try object element
                        this.tryObjectFallback(url);
                        streamLoaded = true;
                    } catch (objectError) {
                        console.warn('فشل object:', objectError);
                        
                        // Show error message with helpful information
                        this.showVideoError(`
                            <div style="text-align: center; padding: 20px;">
                                <h4>🚫 لا يمكن تشغيل هذا الرابط في المتصفح</h4>
                                <p>هذا النوع من روابط IPTV قد لا يكون مدعوماً في المتصفحات.</p>
                                <div style="margin: 15px 0;">
                                    <strong>الحلول المقترحة:</strong>
                                    <ul style="text-align: right; margin: 10px 0;">
                                        <li>استخدم برنامج VLC Media Player</li>
                                        <li>جرب متصفح مختلف (Chrome, Firefox, Edge)</li>
                                        <li>تحقق من اتصال الإنترنت</li>
                                        <li>تأكد من صحة الرابط</li>
                                    </ul>
                                </div>
                                <button onclick="window.open('${url}', '_blank')" style="background: #007bff; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">
            ...
            video.addEventListener('error', (e) => {
                ...
                let errorMessage = 'خطأ في تشغيل البث - تحقق من الرابط';
                
                if (error) {
                    switch (error.code) {
                        case MediaError.MEDIA_ERR_ABORTED:
                            errorMessage = 'تم إلغاء تحميل البث';
                            break;
                        case MediaError.MEDIA_ERR_NETWORK:
                            errorMessage = 'خطأ في الشبكة - تحقق من اتصال الإنترنت';
                            break;
                        case MediaError.MEDIA_ERR_DECODE:
                            errorMessage = 'خطأ في فك تشفير الفيديو - تنسيق غير مدعوم';
                            break;
            ...
            video.addEventListener('error', (e) => {
                console.error('خطأ في تحميل البث:', e);
                streamLoaded = false;
            });
            ...
            if (this.settings.autoplay) {
                try {
                    ...
                    console.warn('لا يمكن التشغيل التلقائي:', playError);
            ...
            const loadingTimeout = setTimeout(() => {
                if (!streamLoaded) {
                    console.warn('انتهت مهلة التحميل، جاري المحاولة بطرق بديلة...');
                    try {
                        this.tryIframeFallback(url);
                        streamLoaded = true;
                    } catch (iframeError) {
                        console.warn('فشل iframe أيضاً:', iframeError);
                        this.showVideoError('انتهت مهلة التحميل. جرب استخدام VLC أو مشغل آخر.');
                    }
                }
            }, 15000); // 15 seconds timeout
            
            // Clear timeout if stream loads successfully
            video.addEventListener('canplay', () => {
    ...
    // Try iframe fallback for IPTV streams
    tryIframeFallback(url) {
        ...
        console.log('تم إنشاء iframe كبديل للبث');
        
        // Store iframe reference for cleanup
        this.currentIframe = iframe;
    }

    // Try WebSocket approach for IPTV streams
    async tryWebSocketFallback(url) {
        console.log('جاري المحاولة بـ WebSocket...');
        ...
        try {
            ...
            ws.onmessage = (event) => {
                console.log('رسالة من WebSocket:', event.data);
            };
            ...
            console.error('فشل WebSocket:', wsError);
            throw wsError;
        }
    }

    // Try Server-Sent Events approach
    async trySSEFallback(url) {
        console.log('جاري المحاولة بـ Server-Sent Events...');
        
        try {
            const eventSource = new EventSource(url);
            
            eventSource.onopen = () => {
                console.log('تم الاتصال بـ SSE');
            };
            
            eventSource.onmessage = (event) => {
                console.log('رسالة من SSE:', event.data);
            };
            ...
            // Store SSE reference for cleanup
            this.currentSSE = eventSource;
            
        } catch (sseError) {
            console.error('فشل SSE:', sseError);
            throw sseError;
        }
    }

    // Try WebRTC approach for IPTV streams
    async tryWebRTCFallback(url) {
        console.log('جاري المحاولة بـ WebRTC...');
        
        try {
            // Create a new video element for WebRTC
            const video = document.getElementById('videoPlayer');
            const videoContainer = document.querySelector('.video-container');
            ...
            webrtcVideo.id = 'webrtcVideo';
            webrtcVideo.controls = true;
            webrtcVideo.autoplay = true;
            webrtcVideo.style.width = '100%';
            webrtcVideo.style.height = '100%';
            
            // Add to container
            videoContainer.appendChild(webrtcVideo);
            
            // Try to use WebRTC for the stream
            const stream = await navigator.mediaDevices.getUserMedia({ video: false, audio: true });
            webrtcVideo.srcObject = stream;
            
            console.log('تم إنشاء WebRTC كبديل للبث');
            
            // Store reference for cleanup
            this.currentWebRTCVideo = webrtcVideo;
            
        } catch (webrtcError) {
            console.error('فشل WebRTC:', webrtcError);
            throw webrtcError;
        }
    }

    // Try object element fallback
    tryObjectFallback(url) {
        console.log('جاري المحاولة بـ object element...');
        ...
        console.log('تم إنشاء object element كبديل للبث');
    ...
    closeModal() {
        ...
        // Remove WebRTC video if exists
        if (this.currentWebRTCVideo) {
            this.currentWebRTCVideo.remove();
            this.currentWebRTCVideo = null;
        }
        
        // Remove object element if exists
